...
def science_analysis_agent():
    ...
    def _run():
        try:
            ...
            workflow = [
                "1. LLM-assisted data reconnaissance and structure exploration: before plotting/statistics, read data-description files, traverse the project directory, inspect delimiters/sample rows/columns, infer schemas and file roles with LLM reasoning, and create or use `data_reconnaissance.md`, `data_schema_manifest.json`, and `io_contracts.md`; then identify which research directions the data can support and follow the user's chosen direction when specified.",
                ...
                "5. Figure and statistics planning: first plan a broad candidate figure/table pool from the study area and available data, then converge later. Candidate artifacts should cover regional context, spatial/depth/time patterns, segmentation, mechanism classes, literature-hypothesis comparison, and robustness when supported.",
                "6. Coding Agent execution: programmatically parse formats, organize fields, write mini tests, compute statistics, and draw candidate figures/tables; every step must declare/check its inputs and outputs; when errors occur, do not stop; diagnose, revise the plan, and retry.",
                "7. Figure-driven scientific iteration: after each batch of plots/statistics, use the emerging visual/statistical patterns to propose sharper scientific questions, then run targeted coding checks again.",
                "8. Evidence synthesis: before writing the paper, integrate data statistics, figures, tables, local papers/RAG, and web search into a claim-evidence-warrant matrix; every scientific conclusion must list supporting evidence, falsification paths, and missing information.",
                "9. Conclusion-driven artifact review: after forming preliminary conclusions, decide which figures/tables should stay in the main text, move to supplement/QC, or be added, then iterate the argument once.",
                "10. Paper writing: write a Markdown paper draft with generated figures embedded via Markdown image syntax; Results are the core and must focus on new mechanistic scientific conclusions, not data quality or figure descriptions.",
                "11. Three-reviewer loop: simulate three strict reviewers covering mechanism novelty, data/statistical reproducibility, and literature evidence/writing; revise until all three are minor-revision/accept or the round limit is reached.",
                "12. Interactive iteration: if the user later proposes modifications, additions, or new hypotheses, continue from the previous results instead of restarting.",
                "13. Keep all reads/writes, scripts, figures, reports, LaTeX, and temporary files inside the user-specified project directory.",
                "14. Use the file profiles above: numeric_text_data/structured_data/waveform_data are candidate data; reference_paper_or_report is literature; data_description_or_project_notes is data documentation. Do not ignore files based only on extension.",
                "15. Parameter-optimization reuse: if the project contains `science_analysis_inputs/parameter_optimization/`, `parameter_optimization_run.md/json`, `best_parameters.json`, `optimization_history.csv`, or `optimization_report.md`, treat them as experimental/method evidence and clearly label dry-run/failure/missing information.",
                "16. Every conclusion must be grounded in data files, statistical outputs, figures, parameter-optimization records, RAG chunks, web literature, or tool output. State evidence gaps explicitly.",
                "17. Do not present intermediate guesses as facts; keep verified/to-be-verified/missing-information status in the report.",
                "18. This is not a data-quality analysis page. Quality labels, field lists, parameter histograms, and basic counts are supplement/QC material only. The main line must be a scientific question such as fault geometry, weak/low-velocity zones, stress transfer, segmented rupture, fluids, or strain-release mechanisms.",
            ] if is_en else [
                "1. LLM 辅助的数据侦察与结构探索：在绘图/统计之前，先读取数据说明文件、遍历项目目录、检查分隔符/样例行/列数，用 LLM 推理文件角色和字段含义，并创建或使用 `data_reconnaissance.md`、`data_schema_manifest.json`、`io_contracts.md`；再判断这些数据可以支持哪些研究方向，用户指定方向时优先服从用户方向。",
                ...
                "5. 图件与统计规划：先根据研究区和可用数据规划广泛的候选图表池，再在后续收敛；候选图表应尽量覆盖研究区背景、空间/深度/时间模式、分段性、机制分类、文献假设对比和稳健性。",
                "6. Coding Agent 执行：自己编程解析数据格式、整理字段、做 mini test、统计并绘制候选图表；每一步都必须声明并检查输入与输出；出错后不要停，必须让 LLM 根据错误自动诊断、改方案、重试。",
                "7. 图后科学迭代：每完成一批图件/统计后，必须根据图形和统计中显露出的模式提出更尖锐的科学问题，再调用编程进行针对性验证。",
                "8. 证据综合：写论文前必须把数据统计、图件、表格、本地论文/RAG 和 web search 整合成 claim-evidence-warrant 矩阵；每个科学结论都要列出支持证据、反证路径和缺失信息。",
                "9. 结论驱动图表复审：形成初步结论后，必须反向判断哪些图表应保留为主文、哪些降为补充/QC、哪些缺口需要补图补表，并据此迭代一次论证。",
                "10. 论文撰写：根据生成图件、统计结果、给定论文和 web search 证据撰写 Markdown 论文草稿，并用 Markdown 图片语法嵌入图件；Results 是文章核心，必须围绕新的机制性科学结论，而不是围绕数据质量或图件说明。",
                "11. 三审稿人循环：模拟 3 个严格审稿人分别审查机制创新、数据/统计可重复性、文献证据与写作；根据意见修订，直到三位均为小修/接收或达到轮次上限。",
                "12. 交互式迭代：如果用户后续提出修改、补充或新假设，基于上一轮结果继续研究，不要从零开始。",
                "13. 所有读写、脚本、图表、报告、LaTeX 和临时文件都必须限制在用户指定的项目工作目录内；不要写到项目目录之外。",
                "14. 必须使用上面的文件画像判断文件作用：numeric_text_data/structured_data/waveform_data 都应视为候选数据；reference_paper_or_report 视为参考文献；data_description_or_project_notes 视为数据说明。不要因为文件扩展名是 .txt/.doc/.pdf 就忽略它。",
                "15. 参数优化复用：如果项目中存在 `science_analysis_inputs/parameter_optimization/`、`parameter_optimization_run.md/json`、`best_parameters.json`、`optimization_history.csv` 或 `optimization_report.md`，必须把它们视为实验/方法证据；可以据此撰写参数优化方法、结果、消融/敏感性分析和论文补充材料，但必须标注 dry-run/失败/缺失信息。",
                "16. 所有结论必须有来源依据：数据文件、统计输出、图件、参数优化运行记录、RAG chunk、web 文献或工具输出。证据不足就明确说明。",
                "17. 不要把中间猜测写成事实；报告中保留“已验证/待验证/缺失信息”状态。",
                "18. 这不是数据质量分析页面：不要把质量分级、字段清单、参数直方图、基础计数作为主线；这些只能进入补充 QC 文件。主线必须围绕科学问题，例如断层几何、弱层/低速体、应力转移、分段破裂、流体或应变释放机制。",
            ]
